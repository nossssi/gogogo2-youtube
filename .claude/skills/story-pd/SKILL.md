...
2. **asset 자동 통과** (2026-07-15 사용자 지시로 검수 게이트 제거) — ASSET_GEN 후 멈추지 않고 자동으로 다음 단계 진행. contact sheet는 결과 보고에 첨부만. 드리프트는 사용자가 사후 지적 → 해당 인물만 lock/anchorProp 수정 후 `--force` 재생성. **사람 게이트는 TTS(Vrew 낭독)와 CapCut 마무리 편집 둘만 남는다.**
...
그 외                                               → DONE
```
asset 검수 게이트 없음(자동 통과) — ASSET_GEN 완료 시 바로 STORYBOARD로.
...
### ASSET_GEN  (자동 — 검수 게이트 없음)
```
python3 scripts/assets/turnaround.py {P}     # 인물 턴어라운드(흰배경 4뷰)
python3 scripts/assets/background.py {P}      # 재등장 장소 시트
```
- 생성 후 contact sheet를 만들어 **결과 보고에 첨부만 하고 멈추지 않고 STORYBOARD로 진행**.
- 드리프트(연령/체형/문화)를 사용자가 사후 지적하면 characters.json의 `build`/`negatives`/`anchorProp` 또는 style.json 앵커를 고쳐 `--force` 재생성.
