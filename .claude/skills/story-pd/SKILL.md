...
2. **검수 게이트** — ASSET_GEN 직후엔 **반드시 사람에게 asset(턴어라운드/배경)을 보여주고 OK를 받는다** (일관성 앵커가 여기서 결정됨). workflow.json mode=auto여도 이 게이트는 유지.
...
그 외                                               → DONE
```
검수 게이트 마커: 별도 파일 없음 — **asset 검수 OK를 받은 뒤에야 storyboard.json을 만들므로**, storyboard.json 존재가 곧 통과 기록.

## 단계별 절차

### SCRIPT  (서브 상태머신, `{S}` = `{P}/_script`)
주력 = **장편 3~4만자 단일 대하 서사** (약 2시간+). 공식(playbook) = `prompts/script-guide.md` — **매 편 벤치마킹하지 않는다**, 공식 현재 버전을 그대로 적용. 단계별로 필요한 프롬프트만 로드(lazy-load):
...
{S}/concept.md 없음                          → CONCEPT ★대화 3박자:
                                              ① 소재 제안 — 스캔 리포트에서 **최적 소재 1개**를 근거(조회수/일,
                                                클러스터 규모)와 함께 제안. topic_log.json 대조(최근 3편과 감정축·
...
{S}/outline.md 없음                          → OUTLINE ★대화: 콜드 오픈 문안 + 장(章) 설계(10~14장 브리프,
...
{P}/script.txt 없음                          → REVIEW: 장 병합 → validate_script.py --target 30000,40000 위반 0 →
...
```
- 생성 후 **contact sheet/개별 이미지를 사용자에게 보여주고 OK를 받는다.**
- 드리프트(연령/체형/문화) 있으면 characters.json의 `build`/`negatives`/`anchorProp` 또는 style.json 앵커를 고쳐 `--force` 재생성.
- 통과 전엔 다음 단계로 가지 않는다.
...
- **★훅·절정은 이미지 밀도를 높인다** (autoworker-youtube 벤치마킹). 훅(첫 막)·재점화·절정 구간은 **문장 1~2개당 한 씬**으로 촘촘히 쪼갠다 — 초반 이탈 방지의 핵심. **씬1은 썸네일급**(가장 강한 시각, 익스트림 구도: 클로즈업·부감·더치앵글). 반대로 배경 설명·잔잔한 전개는 3~5문장을 한 씬으로 묶어도 된다. (교훈: 소금장수 씬1이 문장 6개·30초로 훅인데도 가장 길었다 — 정반대. 훅일수록 짧고 강하게.)
...
- **★장편 청킹 (주력 3~4만자·수백 씬) — 스토리보드도 장 단위로 위임·병합한다.** 한 컨텍스트에서 300+씬을 authoring 하면 컨텍스트 초과·품질 저하. 대본이 `{S}/chapters/`로 나뉘어 있으니 그 경계를 그대로 쓴다.
...
**A) vrew 모드 (반수동, 표준)** — 사용자가 Vrew에서 만든 음성(mp3)+자막(srt)을 `{P}/vrew/`에 넣어 주면 이후는 전부 자동. **단일 나레이션 정책: 대사 포함 전체를 나레이터 한 명이 낭독** (캐릭터별 보이스 분리 없음). 대본 원천은 `script.txt`.
```
